...
function ProductPage({ product, checkoutMutation, variantId }) {
  ...
  const soundcloudLink = product.description.split('#Sc')[1];
      ...
      <div className="flex p-5 align-middle md:items-center">
        <div className="mt-2 mb-8 w-full rounded bg-white p-5 px-4 lg:max-w-7xl">
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="relative mt-4 flex flex-col p-2 md:w-2/5">
              <div className="flex h-full w-full flex-col md:absolute">
                <div className="h-5/6">
                  <h1 className="title-bold m-2">Description:</h1>
                  <p className="text-l m-2 text-gray-500">
                    {product.description}
                  </p>
          ...
          <div className="footer-icons center ... relative w-full md:h-full">
            <h2 className="text-gray text-xs">Updated: {product.updatedAt}</h2>
          </div>
        </div>
      </div>

      <Center className={`${ soundcloudLink ? '' : 'hidden' } w-full p-5 lg:max-w-7xl`}>
        <ReactPlayer width={"100%"} url={soundcloudLink} controls={true} playing={true}/>
      </Center>
    </>
  )
}
